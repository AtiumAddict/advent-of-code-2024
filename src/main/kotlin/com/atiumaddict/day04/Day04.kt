...
fun searchFromX(grid: List<List<Char>>, row: Int, column: Int): Int {
    ...
    for ((nextRow, nextCol) in possibleNextSteps) {
        if (nextRow in grid.indices && nextCol in grid[nextRow].indices && grid[nextRow][nextCol] == 'M') {
            if (searchForNextLetter(grid, nextRow, nextCol, possibleNextSteps.indexOf(Pair(nextRow, nextCol)))) {
                count++
            }
        }
    }
...
fun searchForNextLetter(grid: List<List<Char>>, row: Int, column: Int, direction: Int): Boolean {
    ...
    if (nextRow in grid.indices && nextCol in grid[nextRow].indices && grid[nextRow][nextCol] == letterSequence[letterIndex + 1]) {
