...
val charToAcceptedString = mapOf(
    'm' to "",
    'u' to "m",
    'l' to "mu",
    '(' to "mul"
            ...
            when {
                ...
                char == ')' -> {
                    if (regexForClosingParenthesis.matches(runningMultiplication)) {
                        extractNumbers(runningMultiplication).let {
                ...
                charToAcceptedString.containsKey(char) && charToAcceptedString[char] == runningMultiplication -> {
                    runningMultiplication += char
                }
                else -> {
                    runningMultiplication = ""
...
val charToAcceptedStringForDont = mapOf(
    'd' to "",
    'o' to "d",
    'n' to "do",
    '\'' to "don",
...
val charToAcceptedStringForDo = mapOf(
    'd' to "",
    'o' to "d",
    '(' to "do",
    ')' to "do("
...
private fun handleLineWithDos(line: String, previousResult: Long, enabled: Boolean): Pair<Long, Boolean> {
    var result = previousResult
    var runningMultiplication = ""
    var runningDoOrDont = ""
    var newEnabled = enabled
    for (char in line) {
//        println("Char: $char, Enabled: $newEnabled, runningDoOrDont: $runningDoOrDont, runningMultiplication: $runningMultiplication")
        if (!newEnabled) {
            if (charToAcceptedStringForDo.containsKey(char) && charToAcceptedStringForDo[char] == runningDoOrDont) {
        ...
        when {
            ...
            charToAcceptedStringForDont.containsKey(char) && charToAcceptedStringForDont[char] == runningDoOrDont -> {
                runningDoOrDont += char
            ...
            charToAcceptedStringForDo.containsKey(char) && charToAcceptedStringForDo[char] == runningDoOrDont -> {
                runningDoOrDont += char
