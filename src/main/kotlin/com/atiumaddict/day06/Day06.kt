...
private fun getDirectionToTheRight(direction: Pair<Int, Int>) = directions[(directions.indexOf(direction) + 1) % 4]
...
private fun hasExited(
    nextPosition: Pair<Int, Int>,
    grid: List<List<Char>>
) =
    nextPosition.first < 0 || nextPosition.first >= grid.size || nextPosition.second < 0 || nextPosition.second >= grid[0].size
...
fun solveDay06Second(lines: List<String>): Long {
    ...
    for ((i, node) in path.withIndex()) {
        if (i == 0) continue
        if (isInfiniteLoop(grid, path[i-1].position, node.position, path[i-1].direction, mutableSetOf())) {
...
fun continuePathMarkingObstacles(
    ...
    if (grid[nextPosition.first][nextPosition.second] == '#') {
        ...
        return continuePathMarkingObstacles(grid, nextPositionWithNewDirection, nextDirection, path, obstaclePositions)
    }
